...
S.UI = (function () {
  var input = document.querySelector('.ui-input'),
      ...
      fireText = document.querySelector('.fire-text'),
      interval,
  ...
  var callback = function(){
        ...
        function rotateAll()
        {
          var sideNum = [4,3,5,2,6,1];
          var i = 6;
          var curClass = cubeImageClass;
          function rotateOnce()
          {
            var nextClass = 'show-image-'+sideNum[i];
            cube.classList.replace(curClass, nextClass);
            curClass = nextClass;
          }
          
          function rotateSelfCall()
          {
            if ( (i--) >0){
              setTimeout(rotateSelfCall, 2000);
              rotateOnce();
            }else{
        ...
        rotateAll();
        //放大动画(单独放大container的话,定位会跑偏,所以干脆放大整个body)
